...
Module TwoThreeTrees.
	...
	Module Trees
		...
		Fixpoint left_child (sn : sum (singleton_node_t) (doubleton_node_t))
			: node_t
			:= match sn
			with inl (singleton_node_v (_ as left) (_) (_) (_))
			=> left
			| inr (doubleton_node_v (_ as left) (_) (_) (_) (_) (_) (_))
			=> left
			end.
		...
		Fixpoint value_node (k : Keys.t) (node : node_t)
			...
			=> match KeyLtb.ltb (k) (key_sn (sn))
				with true
				=> value_node (k) (left_child (inl sn))
				| false
